...
class HttpEntity {
  ...
  async list(order = '', limit = 100) {
    ...
    // Map list to frontend representations
...
const qrEntity = {
  list: async () => {
    ...
    // Check if we need to render a dummy QR or fetch live generated code
...
export const base44 = {
  auth: {
    ...
    verifyOtp: async ({ email, otpCode }) => {
      // Mock OTP validation bypass by logging in directly or returning dummy token
  ...
  entities: {
    ...
    // Map DonationSubmission to same endpoint for user panel compatibility
